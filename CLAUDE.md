...
- **Omit missing information rather than flagging it as missing.** The bundle is read by a customer-facing agent, so don't write "TBD" / "not confirmed" / "no official menu" / "unconfirmed" style notes — they read out loud as the agent saying "I don't know." If a fact (a price, an exact day, a confirmed detail) isn't known, just leave it out entirely rather than noting its absence. Freshness pointers ("check Meetup for the current lineup") are fine and should stay phrased as a positive tip, not a disclaimer.
