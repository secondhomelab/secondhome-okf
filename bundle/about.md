---
type: Venue
title: Second Home
description: Venue near Fushimi Station, Nagoya — cafe by day, bar by evening, and event space on select days.
resource: https://tabelog.com/en/aichi/A2301/A230102/23094880/
tags: [cafe, bar, nagoya, fushimi, event-space]
timestamp: 2026-07-12T00:00:00+09:00
---

# Location & Access

* Address: Ark Shirakawa Park Building B1F, 2-2-23 Sakae, Naka-ku, Nagoya-shi, Aichi (愛知県名古屋市中区栄2-2-23 アーク白川公園ビルディング B1F)
* Nearest station: Fushimi Station, about a 4-minute walk (~238m)
* No on-site parking; coin parking available nearby

# Hours

Second Home operates in three modes across the day/week:

* **Cafe** (Wednesday–Sunday, 9:00–17:00) — per the Tabelog listing, which covers the cafe side of the business only. See the [cafe menu](/cafe/).
* **Bar** (17:00–23:00) — TBD / unconfirmed, days not yet specified.
* **Event space** — available on select days for private bookings and hosted events; see [Global Friends in Nagoya](#global-friends-in-nagoya) below and the [full events list](/events/) for the current lineup. Other days TBD.
* Closed: Monday, Tuesday (cafe; bar/event-space closures not yet confirmed)

# About

Second Home is a basement-level venue entered through a door disguised as a painting. It currently functions as a cafe by day, a bar by evening, and an event space on select days. Reviewers describe it as a whimsical "hidden gem" hideout with secret/private rooms and a playful, stylish, movie-like atmosphere. It was featured on TV shortly after opening. Live piano performances are held on weekends — see [Weekend Piano](/events/weekend-piano.md).

# History

* Originally, Second Home was just a rented space where the [Global Friends in Nagoya](https://www.meetup.com/global-friends/) community organized events — there was no bar or cafe.
* The space later began operating as a bar in the evenings.
* Most recently, it also started operating as a cafe during the day. The Tabelog cafe listing shows an opening date of February 28, 2026 for this daytime cafe service — that date marks the start of the cafe, not the founding of the venue itself.
* The cafe operates independently from Global Friends in Nagoya — it is a separate business, not a Global Friends activity. The two share the same owners, which is why they share the same physical space.

# Seating & Facilities

* Counter seating in a stylish, relaxing space
* Private rooms available; private use / venue rental available
* Non-smoking
* Family-friendly and solo-diner friendly

# Budget

¥1,000–¥1,999 per person (lunch and dinner)

# Payment Methods

Credit cards (Visa, Mastercard, JCB, AMEX), IC cards (Suica, Rakuten Edy, iD, QUICPay), PayPay

# Social Media

Second Home's own channels:

* Instagram (cafe): [@second_home_nagoya_cafe](https://www.instagram.com/second_home_nagoya_cafe/)
* Instagram (events): [@second_home_nagoya_events](https://www.instagram.com/second_home_nagoya_events/)

# Global Friends in Nagoya

Events at Second Home are, for practical purposes, Global Friends in Nagoya events; the cafe is a separate, independently-run business under the same ownership (see [History](#history)). Global Friends is a community of 4,923 members (4.7★ from 1,172 ratings, as of July 2026) for people in the Nagoya area to meet, socialize, and hang out.

* Meetup: [Global Friends in Nagoya](https://www.meetup.com/global-friends/)
* Facebook group: [Global Friends in Nagoya](https://www.facebook.com/groups/3384003311915306)
* Current schedule: [Global Friends on Meetup](https://www.meetup.com/global-friends/events/) (also as [RSS](https://www.meetup.com/global-friends/rsvps/rss/)) — recurring weekly/monthly slots (see [Events](/events/)) rotate through different themed nights. Any dates/themes recorded in this bundle are illustrative examples only and go stale fast; always check Meetup for what's actually on.

# Other Event Organizers

Independent communities that also host or co-host events at Second Home, separate from Global Friends:

* [Aichi Connection](https://www.facebook.com/AichiConnection/) (Facebook) — specific events not yet confirmed.
* [Cosmo International Party Nagoya](https://www.meetup.com/ja-jp/cosmo-international-party-nagoya/) (Meetup, 894 members, 4.6★ from 104 reviews as of July 2026) — see [Cosmo International Party](/events/cosmo-international-party.md).

# Reputation

As of July 2026, Second Home holds a 3.14/5 rating from 27 reviews on Tabelog. Reviewers frequently mention the disguised entrance door and underground "hidden cafe" atmosphere. See the [cafe menu](/cafe/) for dishes mentioned in reviews.

# Citations

[1] [Second Home on Tabelog](https://tabelog.com/en/aichi/A2301/A230102/23094880/) — covers cafe hours and reviews only.
[2] [Second Home reviews on Tabelog](https://tabelog.com/en/aichi/A2301/A230102/23094880/dtlrvwlst/)
[3] Bar hours and event-space operation provided directly by the venue operator, 2026-07-12 (unconfirmed / TBD).
